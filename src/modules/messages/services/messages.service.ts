...
export class MessagesService {
  ...
  //     throw new BadRequestException(`Expert ID not found`);
  //   }
  //   if (!payload?.solicitorId) {
  //     throw new BadRequestException(`Solicitor ID not found`);
  //   }

  //   const pageNumber = Math.max(1, Math.floor(payload.page || 1));
  //   const pageLength = Math.max(1, Math.floor(payload.limit || 10)); // Replace 10 with a DEFAULT_LIMIT if you have one
  //   const offset = (pageNumber - 1) * pageLength;

  //   const whereCondition = {
  //     caseId: payload.caseId,
  //     expertId: payload.expertId,
  //     solicitorId: payload.solicitorId,
  //   };

  //   // Total count for pagination
  //   const total = await Messages.count({ where: whereCondition });

  //   const maxPageNumber = Math.ceil(total / pageLength);
  ...
  //       plainMessage.senderRole =
  //         plainMessage.senderId === plainMessage.solicitorId
  //           ? 'solicitor'
  //           : 'expert';
  ...
  //             document.documentName
  //           );
  //           const path = document.documentName.split('/').pop();
  ...
  //       return {
  //         ...plainMessage,
  //         messageDocuments: updatedMessageDocuments,
  //       };
  //     })
  //   );
